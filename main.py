   ...
   try:
        ...
        if call.data == 'check':
            if ch == True:
                ...
                user = str(user_id)
                bot.delete_message(call.from_user.id, call.message_id)
                data['call'][user] = True
...
def unban(message):
    try:
        ...
        bot.send_message(
            message.chat.id, "This user is may not banned if you not sure you can call our dev @soulgoku")
        return
...
def send_text(message):
   try:
    ...
    if str(message.chat.id) not in bdata['call']:
      bdata['call'][str(message.chat.id)] = False
      json.dump(bdata, open('paytmusers.json', 'w'), indent=4)
    if bdata['call'][str(message.chat.id)] == True:
